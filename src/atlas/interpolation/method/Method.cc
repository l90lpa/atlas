...
ExecutionSpace getSparseBackendExecutionSpace(const sparse::Backend& backend) {
...
void fetch(const atlas::Field& f, ExecutionSpace executionSpace) {
    if(executionSpace == ExecutionSpace::Host) {
        if (f.hostNeedsUpdate()) {
            f.updateHost();
        }
    }
    else {
        ATLAS_ASSERT(executionSpace == ExecutionSpace::Device);
...
void flagDesynced(const atlas::Field& f, ExecutionSpace executionSpaceWas) {
    if(executionSpaceWas == ExecutionSpace::Host) {
        f.setDeviceNeedsUpdate(true);
    }
    else {
        ATLAS_ASSERT(executionSpaceWas == ExecutionSpace::Device);
...
void Method::interpolate_field_rank1(const Field& src, Field& tgt, const Matrix& W) const {
    ...
    const auto executionSpace = getSparseBackendExecutionSpace(backend);

    auto src_v = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 1>(src) : make_device_view_fetched<Value, 1>(src);
    auto tgt_v = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 1>(tgt) : make_device_view_fetched<Value, 1>(tgt);
    ...
    flagDesynced(tgt, executionSpace);
}
...
void Method::interpolate_field_rank2(const Field& src, Field& tgt, const Matrix& W) const {
    ...
    if (nonLinear_(src)) {
        ...
        for (idx_t lev = 0; lev < src_v.shape(1); ++lev) {
            ...
            fetch(tgt_slice, ExecutionSpace::Host);
            for (idx_t i = 0; i < tgt.shape(0); ++i) {
                tgt_v(i, lev) = tgt_slice_v(i);
            }
            flagDesynced(tgt, ExecutionSpace::Host);
        }
    }
    else {
        const auto executionSpace = getSparseBackendExecutionSpace(backend);
        auto src_dv = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 2>(src) : make_device_view_fetched<Value, 2>(src);
        auto tgt_dv = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 2>(tgt) : make_device_view_fetched<Value, 2>(tgt);
...
void Method::adjoint_interpolate_field_rank1(Field& src, const Field& tgt, const Matrix& W) const {
    ...
    flagDesynced(src, executionSpace);
}
...
void Method::adjoint_interpolate_field_rank2(Field& src, const Field& tgt, const Matrix& W) const {
    ...
    auto src_v = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 2>(src) : make_device_view_fetched<Value, 2>(src);
    auto tgt_v = (executionSpace == ExecutionSpace::Host) ? make_host_view_fetched<Value, 2>(tgt) : make_device_view_fetched<Value, 2>(tgt);
...
void Method::adjoint_interpolate_field_rank3(Field& src, const Field& tgt, const Matrix& W) const {
    ...
    flagDesynced(src, ExecutionSpace::Host);
}
